...
{ 
  ...
  if ( !client.connected() )
  { Serial.println("> Initialize ELEVENLABS Server connection ... ");
    client.setInsecure();
    ...
    Serial.println("Done. Connected to ELEVENLABS Server.");
  }
  ...
  {  Serial.println( "* ERROR - No AUDIO data for transcription found!");
     return ("");
  }
  ...
  Serial.println("> POST Request to ELEVENLABS Server started, sending WAV data now ..." );
  ...
  if (PSRAM != NULL && PSRAM_length > 0)
  {
     ...
     { if ( (chunk_start + blocksize) > PSRAM_length )   
       ...
       Serial.println( "  Send Bytes: " + (String) blocksize + ", Confirmed: " + (String) result_bytes_sent );  
       chunk_start += blocksize;
     }
     Serial.println( "> All WAV data in PSRAM sent [" + (String) PSRAM_length + " bytes], waiting transcription" );    
  }
  ...
  Serial.println( "\n---------------------------------------------------" );
  /* Serial.println( "-> Total Response: \n" + response + "\n");  // ## uncomment to see the complete server response ##   */
  Serial.println( "ELEVENLABS: [scribe_v1] - " + ((language == "")? ("MULTI lingual") : ("MONO lingual [" + language +"]")) );
  Serial.println( "-> Latency Server (Re)CONNECT [t_connected]:   " + (String) ((float)((t_connected-t_start))/1000) );   
  Serial.println( "-> Latency sending WAV file [t_wavbodysent]:   " + (String) ((float)((t_wavbodysent-t_connected))/1000) );   
  Serial.println( "-> Latency ELEVENLABS response [t_response]:   " + (String) ((float)((t_response-t_wavbodysent))/1000) );   
  Serial.println( "=> TOTAL Duration [sec]: ..................... " + (String) ((float)((t_response-t_start))/1000) ); 
  /*Serial.println( "=> Server detected audio length [sec]: " + wavduration ); // json object does not exist in ElevenLabs */
  Serial.println( "=> Server response length [bytes]: " + (String) response_len );
  Serial.println( "=> Detected language (optional): [" + detect_language + "]" );  
